...
fn main() {
    ...
    let n_entities = world.stars.len();
...
fn tick(world: &mut World) {
    ...
    for planet_data in world.planet.iter_mut() {
        if let Some(planet_data) = planet_data {
            ...
            let orbit_turn_angle =
                (2.0 * PI) / (world.tick_number % planet_data.period_secs) as f64;
            ...
            let _ = mem::replace(&mut planet_data.pos, pos);
        }
    }

    // Аналогично - луны после планет
    for moon_data in world.moon.iter_mut() {
        if let Some(moon_data) = moon_data {
            ...
            let orbit_turn_angle = (2.0 * PI) / (world.tick_number % moon_data.period_secs) as f64;
            ...
            let pos = tilted_rotated_radius_vector + host_pos.pos;
            let _ = mem::replace(&mut moon_data.pos, pos);
        }
    }
}
